		...
		for (Unit unit : units) {
			if (unit.getMainUnit() != null)
				for (String unitName : unit.getMainUnit().getAbbrevations()) {
					 ...
					 */
					String abbr = unit.getMainUnit().getMatchingAbbreviation(
							text);
					if (abbr != null)
...
//
//		UnitManager mgr = new UnitManager();
//		for (Unit u : mgr.getUnits()) {
//			try {
//				if (u.getMainUnit() != null) {
//					node = om.createObjectNode();
//					node.put("UniteName", u.getName());
//					node.put("BaseUnit", u.getMainUnit().getName());
//				} else {
//					node = om.createObjectNode();
//					node.put("UniteName", u.getName());
//					node.put("BaseUnit", "Currency");
//				}
//				ArrayNode subs = om.createArrayNode();
//				for (SubUnit subUnit : u.getSubunits()) {
//					subs.add(subUnit.getName());
//				}
//				node.put("SubUnits", subs);
//				an.add(node);
//
//			} catch (Exception e) {
//				// TODO: handle exception
...
//		}
//		try {
//			om.writeValue(new File("UnitsTest.json"), an);
//		} catch (Exception e) {
