	...
	implements java.io.Serializable
{
	...
	private DataType.ColumnDataType dataType;
	...
	public Map<DataType.ColumnDataType, Integer> getColumnValuesTypes() {
		return columnValuesTypes;
	}

	public void setColumnValuesTypes(
			Map<DataType.ColumnDataType, Integer> columnValuesTypes) {
	...
	private Map<DataType.ColumnDataType, Integer> columnValuesTypes;
	...
	public DataType.ColumnDataType getDataType() {
		return dataType;
	}

	public void setDataType(DataType.ColumnDataType dataType) {
	...
	public TableColumn() {
		...
		header = "";
		dataType = DataType.ColumnDataType.string;
		valuesInfo = new HashMap<String, Integer>();
		columnValuesTypes = new HashMap<DataType.ColumnDataType, Integer>();
	...
	public void setFinalDataType() {
		setDataType(DataType.ColumnDataType.string);
		if (columnValuesTypes.size() > 0) {
			int maxCount = columnValuesTypes.values().iterator().next();
			DataType.ColumnDataType type = columnValuesTypes.keySet().iterator().next();
			for (Entry<DataType.ColumnDataType, Integer> entry : columnValuesTypes
					.entrySet()) {
			...
			// binary
			if (type == DataType.ColumnDataType.bool) {
				for (String val : values.values())
					if (!val.equals("0") && !val.equals("1")
							&& !val.toLowerCase().equals("true")
							&& !val.toLowerCase().equals("false")) {
						type = DataType.ColumnDataType.numeric;
					}
			}
			setDataType(type);
			if (type == DataType.ColumnDataType.unit) {
				setFinalUnit();
			}
		}
	...
	public void addNewDataType(DataType.ColumnDataType valueType) {
