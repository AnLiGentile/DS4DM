...
public class ColumnTypeGuesser {
	...
	public ColumnDataType guessTypeForValue(String columnValue,
		...
		if (columnValue.matches("^\\{.+\\|.+\\}$"))
			return DataType.ColumnDataType.list;
		// check the length
		...
		if (useUnit) {
			...
			if (unitS != null) {
				...
				return DataType.ColumnDataType.unit;
			}
		}
		if (validLenght)
			try {
				...
				if (date != null)
					return DataType.ColumnDataType.date;
			} catch (Exception e) {

			}

		if (validLenght && Boolean.parseBoolean(columnValue))
			return DataType.ColumnDataType.bool;

		if (URLParser.parseURL(columnValue))
			return DataType.ColumnDataType.link;

		if (validLenght && GeoCoordinateParser.parseGeoCoordinate(columnValue))
			return DataType.ColumnDataType.coordinate;

		if (validLenght && NumericParser.parseNumeric(columnValue)) {
			return DataType.ColumnDataType.numeric;
		}

		return DataType.ColumnDataType.string;
